...
void CMainWindow::Render()
{
	...
	if (ImGui::Begin("Nexus", (bool*)0, Flags | (this->IsHandleHeld ? 0 : ImGuiWindowFlags_NoMove)))
	{
		...
		if (ImGui::BeginChild("Footer", ImVec2(contentWidth, footerHeight), false, Flags))
		{
			...
			ImGui::SetCursorPos(ImVec2(((contentWidth - ImGui::CalcTextSize((const char*)u8"© 2021 - 2025 Raidcore").x) / 2) - sidebarWidth, (footerHeight - ImGui::GetFontSize()) / 2));
			ImGui::Text((const char*)u8"© 2021 - 2025 Raidcore");
		}
		ImGui::EndChild();
